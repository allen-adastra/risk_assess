...
def assess_risk_gmms(body_frame_gmm_traj, Q, method, **kwargs):
    """
    ...
    """
    tstart = time.time()
    t = 1.0 # We are interested in x'Qx <= 1
    risk_estimates = [1 - GmmQuadForm.upper_tail_probability(gmm, Q, t, method, **kwargs) for gmm in body_frame_gmm_traj]
    t_total = time.time() - tstart
    return risk_estimates, t_total
